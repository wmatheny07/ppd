{{ config(
    ...
    schema='analytics',          -- change if you use a different schema
    unique_key='play_stat_pk',   -- optional; doesn't control MV, but helps consistency
    indexes=[
...
with base as (
  select
    ...
    p.event_id,
    p.competition_id
from {{ source('espn_raw', 'espn_play_stat') }} ps
join {{ source('espn_raw', 'espn_play') }} p
    on p.id = ps.play_id
),
